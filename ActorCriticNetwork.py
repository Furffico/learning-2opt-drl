...
class Encoder(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        edges = utils.batch_pair_squared_dist(input, input) # 距离矩阵（重复计算了？）
        edges.requires_grad = False
        # edges: (B x n_nodes x n_nodes )

        # *--------------------------------- embedding --------------------------------* #
        # embedding shared across all nodes
        embedded_input: torch.Ternsor = self.embedding(input)
        # embedding_input: (B x n_nodes x embedding_dim)
        ...
        # g_embedding: (B x n_nodes x hidden_dim)

        # *----------------------------------- LSTM -----------------------------------* #
        if hidden is None:
            h0 = self.h0.unsqueeze(0).unsqueeze(0).repeat(self.n_rnn_layers, batch_size, self.hidden_dim)
            c0 = self.h0.unsqueeze(0).unsqueeze(0).repeat(self.n_rnn_layers, batch_size, self.hidden_dim)
        else:
            h0, c0 = hidden
            h0 = h0.detach().unsqueeze(0).repeat(self.n_rnn_layers, 1, 1)
            c0 = c0.detach().unsqueeze(0).repeat(self.n_rnn_layers, 1, 1)
        ...
        s_out_reversed, s_hidden_reversed = self.rnn_reversed(rnn_input_reversed, (h0_r, c0_r))

        # *---------------------------------- output ----------------------------------* #
        # combine output of two LSTMs
        s_out = torch.tanh(self.W_f(s_out) + self.W_b(torch.flip(s_out_reversed, [1])))
        s_hidden = (s_hidden[0]+s_hidden_reversed[0], s_hidden[1]+s_hidden_reversed[1])
        # s_out: (B x n_nodes x hidden_dim)
        # s_hidden: h=(n_rnn_layers x B x hidden_dim), c=(n_rnn_layers x B x hidden_dim)
...
class Attention(nn.Module):
    ...
    def __init__(self, hidden_dim, C=10.0, T=1.0):
        ...
        """
        self.W1 = nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=1, stride=1, bias=False)
        ...
        # Initialize vector V
        torch.nn.init.uniform_(self.V.weight, a=-1.0/math.sqrt(hidden_dim), b=1.0/math.sqrt(hidden_dim))
        self._inf = float('-inf')

    def forward(self, ref, q, mask=None):
        """
        ...
        ref_W1 = self.W1(ref).permute(0, 2, 1)

        # q_W2: (batch_size, 1, hidden_dim)
        q_W2 = self.W2(q).unsqueeze(1)

        # u_i: (batch_size, n_nodes)
        u_i = self.V(torch.tanh(ref_W1 + q_W2)).squeeze(-1).masked_fill_(mask, self._inf)
        u_i = self.C * torch.tanh(u_i/self.T)

        # probs: (batch_size, n_nodes)
        probs = F.softmax(u_i, dim=1)

        # q_a: (batch, hidden_dim)
        q_a = torch.bmm(probs.unsqueeze(1), ref_W1).squeeze(1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, n_actions):
        ...
        self.init_dec = nn.Parameter(torch.FloatTensor(hidden_dim), requires_grad=False)
        nn.init.uniform_(self.init_dec, -1/math.sqrt(hidden_dim), 1/math.sqrt(hidden_dim))
    ...
    def forward(self, q, ref, inp, actions=None, g_emb=None, q_star=None):
        ...
        # runner: (batch, seq_len) filled with {0,...,seq-len-1}
        runner = torch.arange(0, n_nodes).unsqueeze(0).expand(batch_size, -1).long()
        ...
        for i in range(self.n_actions):
            ...
            h = torch.tanh(self.W_1(h) + self.W_0(dec_input))
            ...
            mask = mask * (1 - (runner <= indices).float())

            # embbeding mask: boolean (batch size, seq_len, embbeding_dim)
            # True for the pointed input False otherwise
            dec_input_mask = (runner == indices).unsqueeze(2).expand(-1, -1, self.hidden_dim)
...
class ActorCriticNetwork(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(input_dim, embedding_dim, hidden_dim, n_nodes, n_rnn_layers)      # encoder for S
        self.encoder_star = Encoder(input_dim, embedding_dim, hidden_dim, n_nodes, n_rnn_layers) # encoder for S'
        self.decoder_a = Decoder(embedding_dim, hidden_dim, n_actions)                           # policy decoder
        self.decoder_c = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 1))   # value decoder
    ...
    def forward(self, inputs, inputs_star, hidden=None, actions=None):
        ...
        probs, pts, log_probs_pts, entropies = self.decoder_a(enc_h, s_out, s_out, actions, g_embedding, enc_h_star)
